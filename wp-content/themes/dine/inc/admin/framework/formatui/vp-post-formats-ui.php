...
function vp_pfui_add_meta_boxes($post_type) {
	if (post_type_supports($post_type, 'post-formats') && current_theme_supports('post-formats')) {
		// assets
		wp_enqueue_script('dine', vp_pfui_base_url().'js/admin.js', array('jquery'), VP_PFUI_VERSION);
		wp_enqueue_style('dine', vp_pfui_base_url().'css/admin.css', array(), VP_PFUI_VERSION, 'screen');

		wp_localize_script(
			'dine',
			'vp_pfui_post_format',
			array(
				'loading'      => esc_html__('Loading...', 'dine'),
				'wpspin_light' => admin_url('images/wpspin_light.gif'),
				'media_title'  => esc_html__('Pick Gallery Images', 'dine'),
				'media_button' => esc_html__('Add Image(s)', 'dine')
			)
		);
...
function vp_pfui_post_admin_setup() {
	...
	if (!empty($post_formats[0]) && is_array($post_formats[0])) {
		...
		include get_template_directory() . '/inc/admin/framework/formatui/views/tabs.php';
		...
		foreach ($format_views as $format) {
			if (in_array($format, $post_formats)) {
				include get_template_directory() . '/inc/admin/framework/formatui/views/format-'.$format.'.php' ;
			}
		}
	}
}
...
function vp_pfui_gallery_preview() {
	...
	$post->ID = $post_id;
	ob_start();
	include get_template_directory() . '/inc/admin/framework/formatui/views/format-gallery.php';
	$html = ob_get_clean();
