...
(function (window) {
    ...
    Jarallax.prototype.init = function () {
        ...
        // wrapper
        jQuery( _this.$item ).find( '.wpb_content_element, .vc_custom_heading, .dine-button, .dine-icon-wrapper' ).each(function( i ){
            
            var $this = jQuery( this );
            
            if ( $this.parent().is( '.dine-parallax-element' ) ) return;
            
            $this.wrap( '<div class="dine-parallax-element" />' );
            
            if ( $this.is( '.dine-button' ) && $this.parent().next().is( '.dine-button' ) ) {
    ...
    Jarallax.prototype.onScroll = function (force) {
        ...
        if(_this.options.type === 'scroll' || _this.options.type === 'scroll-opacity') {
            ...
            if(supportTransform && _this.options.enableTransform) {
                ...
                // styles.transform = 'matrix(1, 0, 0, 1, 0, ' + positionY + ')';
        ...
        TweenMax.to( jQuery( _this.image.$item ), 0, { y: positionY } );
        
        jQuery( _this.$item ).find( '.dine-parallax-element' ).each(function( i ){
          
            var $this = jQuery( this ),
                eleRect   = this.getBoundingClientRect(),
                contT  = $this.offset().top,
                factor  = contT / rect.height,
                opacity = 1,
                trans = 0,
                styles = {
                };
            
            if ( fromViewportCenter >=0 ) {
                opacity = 1;
                trans = 0;
            } else {
                opacity = Math.max( ( 1 - Math.pow( -fromViewportCenter, 3 )* 3 ), 0 );
                trans = - fromViewportCenter * _this.parallaxScrollDistance / ( i + 3 ) * ( i + 1 );
            }
            
            styles.opacity = opacity;
            
            // styles.transform = 'translate3d(0, ' + positionY + 'px, 0)';
            styles.transform = 'matrix(1, 0, 0, 1, 0, ' + Math.round( trans ) + ')';
    ...
    addEventListener(window, 'load', loop);
    addEventListener(window, 'jarallax-reInit', update);
